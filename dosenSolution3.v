...
Module Type Solution.
  ...
  | ModCom : forall {UACom}, forall A3, UACom ⊢a A3 -> forall A1, A1 ⊢a UACom -> A1 ⊢a A3
  ...
  | CoModCom : forall {UBCom}, forall B3, UBCom ⊢b B3 -> forall B1, B1 ⊢b UBCom -> B1 ⊢b B3
  ...
  (** Definitional right-composition *)
  
  Notation " 'φ' a> f " := (CoUnit f) (at level 26, right associativity) : solScope.
  Notation " 'γ' b> g " := (Unit g) (at level 26, right associativity) : solScope.
  Definition ModComR : forall {UACom}, forall A1, A1 ⊢a UACom -> forall A3, UACom ⊢a A3 -> A1 ⊢a A3
    := fun UACom A1 f1 A3 f2 => ModCom f2 f1.  
  Notation "f1 a> f2" := (ModComR f1 f2) (at level 25, right associativity) : solScope.
  Definition CoModComR : forall {UBCom}, forall B1, B1 ⊢b UBCom -> forall B3, UBCom ⊢b B3 -> B1 ⊢b B3
    := fun UBCom B1 g1 B3 g2 => CoModCom g2 g1.
  Notation "g1 b> g2" := (CoModComR g1 g2) (at level 25, right associativity) : solScope.
  
  Check  ( γ b> G| ( φ a> F| G| @ModIden F|0 '#0 _ ) ) b> G| F| '1 .
  Check  γ b> ( G| ( φ a> F| G| @ModIden F|0 '#0 _ ) b> G| F| '1 ) .
  Check  γ b> G| ( φ a> F| G| @ModIden F|0 '#0 _ ) b> G| F| '1 .
  ...
  Section Yoneda.
    Variable coerceSet_ModObjects : ModObjects -> Set.
    Coercion coerceSet_ModObjects : ModObjects >-> Sortclass.
    Variable coerceFun_ModArrows : forall A1 A2, (A1 ⊢a A2) -> (A1 -> A2).
    Coercion coerceFun_ModArrows : ModArrows >-> Funclass.
    Definition Yoneda_Refl : forall (B1 : CoModObjects), F|0 B1 -> forall (B2 : CoModObjects), (B1 ⊢b B2) -> F|0 B2 :=
 ...
 Are there alternative solutions? *)
  ...
  |  ModCongCoUnit : forall A1 A2, forall (f f' : A1 ⊢a A2), f ≈a f' -> φ a> f ≈a φ a> f'
  ...
  |  ModCat1Right : forall A1 A2, forall f : A1 ⊢a A2,  f ≈a 1 a> f
  ...
  |  CoModCongCoUnit : forall B1 B2, forall (g g' : B1 ⊢b B2), g ≈b g' -> γ b> g ≈b γ b> g'
  ...
  |  CoModCat1Right : forall B1 B2, forall g : B1 ⊢b B2,  g ≈b '1 b> g
  ...
  | rightComMod : forall A1 A2 (f1 : A1 ⊢a A2), nodesMod f1 -> forall {A3} {f2 : A2 ⊢a A3}, nodesMod (f2 <a f1)
   ...
   ... memo that another name for filtered colimit is inductive colimit !  **) 
    ...
    Proof.
      apply redCongMod_Mod_redCongMod_CoMod_mutind;
      eauto.
      destruct 1. apply ModSym, ModCat2.
    Qed.
    ...
    Proof.
      apply redCongCoMod_Mod_redCongCoMod_CoMod_mutind;
      eauto.
      destruct 1; apply CoModSym, CoModCat2.
    Qed.
